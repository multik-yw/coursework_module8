...
class TelegramBot:
    ...
    def send_message(chat_id, text):
        ...
        try:
            response = requests.post(url, params=params, timeout=(5, 10))
...
def send_telegram_notification(self, telegram_id, message):
    ...
    try:
        result = TelegramBot.send_message(chat_id=int(telegram_id), text=message)
...
def check_habits_and_notify():
    ...
    habits = (
        Habit.objects.filter(time__hour=current_time.hour, time__minute=current_time.minute)
        .exclude(last_completed__date=now.date())
        .select_related("user")
    )
    ...
    for habit in habits:
        if habit.user.telegram_id:
            ...
            send_telegram_notification.delay(habit.user.telegram_id, message)
